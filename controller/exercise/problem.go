...
func (this *ProblemController) List(w http.ResponseWriter, r *http.Request) {
	log.Println("Exercise Problem List")
	this.InitExercise(w, r)

	if (this.GetTime() < this.ExerciseDetail.Start || this.ExerciseDetail.Status == config.StatusReverse) && this.Privilege <= config.PrivilegePU {
		...
		this.Data["Info"] = "The Exercise has not started yet"
		if this.ExerciseDetail.Status == config.StatusReverse {
			this.Data["Info"] = "No such Exercise"
		}
	...
	list := make([]problem, len(this.ExerciseDetail.List))
	...
	t, err := t.ParseFiles("view/layout.tpl", "view/exercise/problem_list.tpl")
	if err != nil {
		log.Println(err)
		http.Error(w, "tpl error", 500)
		return
	...
	this.Data["IsExerciseProblem"] = true
	err = t.Execute(w, this.Data)
	if err != nil {
		log.Println(err)
		http.Error(w, "tpl error", 500)
...
func (this *ProblemController) Detail(w http.ResponseWriter, r *http.Request) {
	log.Println("Exercise Problem Detail")
	this.InitExercise(w, r)

	if (this.ExerciseDetail.Status == config.StatusReverse || this.GetTime() < this.ExerciseDetail.Start) && this.Privilege <= config.PrivilegePU {
		t := template.New("layout.tpl")
		t, err := t.ParseFiles("view/layout.tpl", "view/400.tpl")
		if err != nil {
			log.Println(err)
	...
	response, err := http.Post(config.PostHost+"/problem/detail/pid/"+strconv.Itoa(this.ExerciseDetail.List[pid]), "application/json", nil)
	if err != nil {
		http.Error(w, "post error", 500)
		return
	}
	...
	this.Data["Pid"] = pid
	this.Data["Status"] = this.ExerciseDetail.Status

	t := template.New("layout.tpl").Funcs(template.FuncMap{
		"ShowRatio":   class.ShowRatio,
		"ShowStatus":  class.ShowStatus,
		"ShowSpecial": class.ShowSpecial})
	t, err = t.ParseFiles("view/layout.tpl", "view/exercise/problem_detail.tpl")
	if err != nil {
		http.Error(w, "tpl error", 500)
		return
	}
...
func (this *ProblemController) Submit(w http.ResponseWriter, r *http.Request) {
	log.Println("Exercise Problem Submit")
	this.InitExercise(w, r)
	...
	pid = this.ExerciseDetail.List[pid]
	uid := this.Uid
	if uid == "" {
		w.WriteHeader(401)
		return
	...
	one["pid"] = pid
	one["uid"] = uid
	one["mid"] = this.ExerciseDetail.Cid
